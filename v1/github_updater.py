...
def count(seq, start=0):
    scores = [0.5] * 15
    for value in seq[start:]: scores[value - 1] += 1
    return norm(scores)
...
def markov1(seq):
    ...
    if seq:
        ...
        for i in range(1, len(seq)):
            if seq[i - 1] == last: scores[seq[i] - 1] += 1
    return norm(scores)

def lag(seq, n):
    scores = [0.5] * 15
    if len(seq) >= n: scores[seq[-n] - 1] += 4
    return norm(scores)
...
def gap_hazard(seq):
    ...
    for i, value in enumerate(seq): positions[value].append(i)
    end = len(seq) - 1
    for number in range(1, 16):
        ...
        ratio = current_gap / max(mean_gap, 1.0)
        scores[number - 1] += min(4.0, ratio)
        if current_gap <= 2: scores[number - 1] += 0.35
    return norm(scores)
    ...
    for length, weight in ((6, 3.0), (5, 2.4), (4, 1.8), (3, 1.2)):
        ...
        for i in range(length, len(seq)):
            if seq[i - length:i] == suffix:
                ...
                found += 1
        if found >= 3: break
    if not found: return decay_frequency(seq, 0.94)
    return norm(scores)

def run_state(seq):
    scores = [0.35] * 15
    if not seq: return norm(scores)
    last = seq[-1]
...
def cycle_bank(seq):
    ...
    for period in periods:
        ...
        for end in range(start, len(seq)):
            partial = seq[:end]
            prediction = top5(cycle(partial, period))
            recent_hits += seq[end] in prediction
            tests += 1
        weight = (recent_hits + 2.0) / (tests + 6.0)
        for i, score in enumerate(scores): combined[i] += score * weight
    return norm(combined)

def fresh_model_weights(): return {key: 1.0 for key in MODEL_NAMES}

def fresh_expert_weights(): return {str(h): 1.0 for h in HORIZONS}
...
def diversify(combined, recent_predictions):
    ...
    for picks in recent_predictions[-10:]:
        for number in picks:
            if 1 <= int(number) <= 15: exposure[int(number) - 1] += 1
    adjusted = list(combined)
    for i, seen in enumerate(exposure): adjusted[i] *= 1.0 - min(0.15, seen * 0.015)
    ...
    if previous and set(candidate) == set(previous):
        ...
        if fifth <= 0 or (fifth - sixth) / fifth < 0.08: candidate[-1] = sixth_number
    return candidate
...
def update_model_weights(weights, model_top5, actual):
    ...
    for key in weights: weights[key] /= average
...
def model_scorecard(history):
    ...
    for key, name in MODEL_NAMES.items():
        ...
        card[key] = {"name": name, "evaluated": evaluated, "successes": successes,
...
def main():
    ...
    if not draws: raise RuntimeError("The original live state has no draw data")
    old = load_json(STATE, {})
    ...
    for row in sorted(new_rows, key=lambda item: item["draw"]):
        if old_pending and int(old_pending.get("draw", -1)) == int(row["draw"]):
            ...
            live.append({
                ...
                "type": "automated_future_test", "predictedAt": old_pending.get("createdAt"), "scoredAt": now(),
            })
    ...
    pending = {
        "draw": latest + 1, "top5": picks, "modelTop5": model_top5,
        "selfLearnerExpertTop5": expert_top5, "createdAt": now(), "feedControlled": True,
    ...
    state = {
        "draws": draws, "history": history, "weights": model_weights,
        "modelScorecard": model_scorecard(history),
        "selfLearner": {"name": MODEL_NAMES["selfLearner"], "expertWeights": expert_weights, "horizons": list(HORIZONS)},
        "pending": pending, "warmup": warmup, "latestDraw": latest, "seedCount": len(draws),
        "feed": "isolated experiment reading verified v0 state", "source": "v0/cloud_state.json",
    ...
    print(json.dumps({"latestDraw": latest, "nextDraw": latest + 1, "prediction": picks,
                      "models": len(MODEL_NAMES), "newModels": 6}))
