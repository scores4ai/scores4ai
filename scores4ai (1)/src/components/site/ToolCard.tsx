      ...
      >
        ...
        <div className="mt-4">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">· {tool.developer}</span>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {tool.tags.slice(0, 3).map((t) => (
            <span key={t} className="rounded-md bg-secondary px-2 py-0.5 text-[10px] text-muted-foreground">
              {t}
            </span>
          ))}
        </div>
